...
| Onboarding | Welcome demo, setup, **goal question** (`profiles.primary_goal`, migration 039), first spoken log, habit step, then a screen that names the four things the trial just switched on. No price on day zero: they already have it | A reverse trial only converts if the user knows they hold the paid product and uses it; a silent trial makes the downgrade invisible. The goal is not decoration: it orders the Today checklist and is quoted on the paywall |
| Trial length | **14 days** | RevenueCat 2026: trials of 17 to 32 days convert at 45.7% median against 26.8% for 3 to 7 days. Our marginal cost is near zero and an expense tracker needs more than a week of its own history to be worth paying for |

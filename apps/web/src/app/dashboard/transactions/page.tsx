...
type FilterKey = 'all' | 'voice' | 'shortcut' | 'notification' | 'recurring' | 'income'
 ...
 *
 *  `shortcut` (an iOS Shortcut) and `notification` (Android's
 *  NotificationListenerService) used to collapse into one "Apple Pay"
 *  label (fix-plan 3.4 / audit 07-F10, 07-F11, 08-F19, 08-F20): wrong on
 *  its face for `notification` — an Android-only source with no
 *  connection to Apple Pay — and imprecise for `shortcut`, which fires
 *  for any iOS Shortcut a user builds, not specifically an Apple Pay one.
 *  Kept as two distinct, honestly-named buckets instead. */
...
export default function TransactionsPage() {
  ...
  // Drives the realtime subscription's filter below — `load()` sets this
  // once `auth.getUser()` resolves.
  ...
  // Realtime — one shared hook, not a hand-rolled effect (fix-plan 4.6).
  // The filter embeds the user's id so Supabase only sends this client the
  // changes it actually needs; it stays `null` (no subscription) until
  // `load()` above resolves `userId`.
  useRealtime('transactions', userId ? `user_id=eq.${userId}` : null, load)
  ...
  const counts = useMemo(() => {
    ...
    for (const t of sourceSet) {
      ...
      if (k === 'voice') voice += 1
      else if (k === 'shortcut') shortcut += 1
      else if (k === 'notification') notification += 1
    ...
    return { voice, shortcut, notification, typed, scan, recurring }
  }, [sourceSet])
        ...
        <div style={styles.headerRow}>
          <div>
            ...
            <div style={styles.subtitleLine}>
              ...
              {counts.shortcut} shortcut · {counts.notification} notification · {counts.recurring} recurring
            </div>
          </div>
              ...
              filtered.map((t, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div>
                      {catName ? (
                        <Chip label={catName} categoryColor={catColor} size="sm" />
                      ) : isIncome ? (
